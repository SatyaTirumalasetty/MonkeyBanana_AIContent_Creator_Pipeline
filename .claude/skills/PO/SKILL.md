...
- [ ] **3 of 7 content types fail almost every time — `educational`, `short_film`, and `custom` failed 4/4 (100%) in a live `/QA` regression batch, vs. 0/2 for `advertisement`.** Verified 2026-06-20: the failure is `"No JSON found in response"` from `generateVideoMetadata` (`src/lib/agents.ts:355-378`), which calls `callGemini` *without* overriding the default `maxOutputTokens: 4096` (`agents.ts:38`). That function's output includes a word-level `lipSyncMap` for the *entire* narration script — for longer-form scripts (explainers, short films, open-ended custom briefs run noticeably longer than a short rhyme or ad's VO lines) the response is large enough to exceed 4096 tokens and gets truncated before any JSON structure survives, so `safeJSON` finds no `{` at all. This happens *after* content/storyboard generation already succeeded (confirmed via SSE log trace — failure is specifically at the "Video Producer Agent" step), meaning the quota slot is already burned too (compounds the item below). Calling this P0 rather than P1 despite not being a literal payment-collection blocker: a paying Studio/Cinema user selecting one of these 3 content types gets a broken product almost every time and burns their budgeted Kling AI slot on a guaranteed failure — that's "money collected for something that doesn't work," which is the spirit of the P0 bar even if not the letter.
  - Fix: pass a much higher `maxOutputTokens` (e.g. 16384+) to the `generateVideoMetadata` call specifically, since its output scales with script length unlike the other agent calls. Effort: one-line change, very high leverage — this is likely the single most valuable fix available right now, ahead of the privacy/ToS page, because it's blocking core product function for ~43% of content types today.
