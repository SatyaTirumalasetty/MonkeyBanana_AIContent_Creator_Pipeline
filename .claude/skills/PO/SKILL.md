...
- [ ] **Free-tier users lose quota slots to a content-generation parsing bug, with no recourse.** Verified via `/QA` exploratory testing against live production (2026-06-20): `checkAndReserveUsage` reserves a video slot *before* content generation runs in `/api/pipeline/start`, and that reservation is never refunded if generation fails downstream. Root cause is `safeJSON`'s brace-counting in `src/lib/agents.ts:11-15`, which isn't string-aware — a literal `{`/`}` inside any generated text field slices the JSON candidate at the wrong position, producing an unrecoverable parse error (`jsonrepair` can't fix a wrong cut point, only malformed-but-complete JSON). Reproduced in 2 of 5 real production calls in one test session (~40% in a small sample — not proof of the true rate, but far too frequent to dismiss). Net effect: a meaningful fraction of free-tier signups burn one of their 3 conversion-critical trial videos on a server bug that produces nothing — directly undermines the monetization fix in `cbac24a`. For paying Studio/Cinema users, the same bug can also burn a budgeted Kling AI slot on a failed render, which is a real-dollar fal.ai cost-control issue, not just a free-tier UX one.
  - Fix, in order of leverage: (1) make `safeJSON`'s extraction string/quote-aware so it stops mis-slicing valid output — this is the actual root cause and likely fixes most failures outright; (2) regardless of (1), only reserve usage *after* a generation attempt succeeds (or refund on failure) so a remaining bug class can't cost users their quota — defense in depth, not a substitute for the real fix.
