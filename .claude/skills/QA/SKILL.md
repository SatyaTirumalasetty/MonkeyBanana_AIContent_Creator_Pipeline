...
- [ ] **`generateVideoMetadata` truncates on longer scripts** (`src/lib/agents.ts:355-378`, no `maxOutputTokens` override, default 4096) — see `/PO` P0. Regression test once fixed: run all 7 content types with a brief that tends to produce a longer script, confirm zero `"No JSON found in response"` failures, and spot-check `lipSyncMap` isn't itself silently truncated (i.e. covers the full script length, not just the first N words).
- [ ] **Storyboard caption overlay still shows raw markdown** (`page.tsx:269`) — see `/PO` P1. Regression test once fixed: same DOM-scan technique (search the whole page for literal `**`, not just the one element you fixed) against an `advertisement`/`educational` generation.
