...
- [ ] **"Video ready" displays before the video finishes rendering** — see `/UX`'s open issue of the same name; reproduced fresh with a screenshot during the 2026-06-21 redesign regression pass (see Verification log above). Root cause: `complete` set on the SSE `complete` event, before `renderVideo()`'s clip-render-and-stitch loop finishes.
