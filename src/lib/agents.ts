...
// Kling AI generates one 5s clip at a time (via fal.ai). A long video is
// assembled from many clips stitched together. Tune via env vars — more clips
// = more Kling API cost and total generation time (each clip takes ~2-4 min).
...
export async function planStoryboard(
  ...
  const system = `You are an animation storyboard planner for a 9:16 vertical kids video (~${totalSec} seconds total, made of ${numShots} shots of ${clipDurationSec}s each). The shots will be generated by Kling AI as real animated video clips.
Return ONLY valid JSON with exactly this shape:
{
  "shots":[
...
- Each shot description must be a clear ACTION sentence (max 15 words) describing what is visually happening — motion, expression, setting. Do NOT describe character appearance (a reference image handles that).
- The first shot MUST be a strong hook showing the main character in an exciting moment.
- Build a visual journey that extends the rhyme's theme across fun mini-scenes — cohesive and age-appropriate for toddlers 2-5.
- Vary emoji and camera angles (close-up, wide shot, medium shot, overhead, tracking shot) across shots.
- mood must be a single adjective like "joyful", "magical", "playful", or "whimsical".
- backgroundStyle must describe the setting like "enchanted forest", "underwater kingdom", or "colorful meadow".`
